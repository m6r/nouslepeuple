...
function getmyurl($x, $var1 = "", $var2 = "", $var3 = "")
{
    ...
    if ($URLMethod == 1) {
        if ($x == "index") {
            ...
            $ret = "/rss.php?category=$var1&amp;status=$var2&amp;group=$var3";
        } elseif ($x == "rss") {
            $ret = "/rss.php";
        } elseif ($x == "rssnew") {
            $ret = "/rss.php?status=new";
        } elseif ($x == "rssall") {
...
function allowToAuthorCat($cat)
{
    ...
    if ($user->level == "admin") {
        ...
        if ($user->level == "moderator" && ((is_array($cat) && $cat['authorlevel'] != "admin") || $cat->category_author_level != "admin")) {
            return true;
        } else {
            if ((is_array($cat) && $cat['authorlevel'] == "normal") || $cat->category_author_level == "normal") {
                // DB 11/12/08
